# This script is used to process the data from batch 1 sequencing. 

library(dada2)
setwd("batch1_syncom_data")

path<-"16S_amplicons_batch1"
list.files(path)
library(ShortRead)
library(Biostrings)
...
out<-filterAndTrim(fnFs, filtFsNS, fnRs, filtRsNS, maxN = 0, truncLen = c(280,200),trimLeft =c(19,18), 
                   rm.phix = TRUE, truncQ = 2, maxEE = c(3,3), compress = TRUE, multithread = TRUE) # for batch 2 I ran with maxEE (2,2)
...
dim(seqtab)
table(nchar(getSequences(seqtab))) #They are between 261 and 421in length. I will keep all, and problematic ones will most 
...
setwd("../../cintia");taxa <- assignTaxonomy(seqtab.nochim, "silva_nr99_v138.1_wSpecies_train_set.fa.gz", multithread=TRUE);setwd("../Frederik/batch1_syncom_data")
...
save.image("batch1_syncom.rData")
saveRDS(taxa,"taxa_batch1.rds")
saveRDS(seqtab.nochim,"asv_table_batch1.rds")
